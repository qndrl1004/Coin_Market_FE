...
export default function Key() {
  ...
  const [inLogin, _setInLogin] = useState(true);
  ...
  const onKeyBtn = () => {
    axios
      .post("/api/info/userkey", { connectKey, secretKey })
      .then((response) => {
  ...
  return inLogin ? (
    <div className="flex justify-center items-center rounded border-2 h-[120px] px-3">
      ...
      <button
        className="rounded bg-blue-500 hover:bg-blue-700 text-white h-[60%] mt-[20px] px-1"
        onClick={onKeyBtn}
      >
        전송
      </button>
