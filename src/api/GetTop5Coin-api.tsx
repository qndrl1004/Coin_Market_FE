    ...
    const fetchTop5CoinsData = async () => {
      try {
        ...
        const sortedCoins = Object.keys(coinData)
          ...
          .sort((a, b) => {
            const aFluctuation = Math.abs(
              parseFloat(coinData[a].fluctate_rate_24H)
            );
            const bFluctuation = Math.abs(
              parseFloat(coinData[b].fluctate_rate_24H)
            );
    ...
    <div>
      ...
      <ul className="w-full">
        {top5Coins.map((coin, index) => (
          <li
            key={index}
            className={`flex my-[4px] mx-auto w-[100%] h-[100%] shadow-md shadow-slate-300 border-slate-200 hover:shadow-lg ${
              darkMode ? "hover:bg-yellow-600" : "hover:bg-yellow-300"
          ...
          >
            ...
            <div className="text-start w-[90px]">
              <p className="">{coin.fluctuation}</p>
            </div>
            <div>
              <p className="text-[16px] mr-[20px]">
                {parseFloat(coin.fluctuation) > 0 ? (
